...
class ScrollNavBar: UIView {  
    ...
    func initTitle(titles : Array<String>,isScroll : Bool) {  
        ...
        //创建标题滚动视图  
        titleScrollView.frame = CGRect.init(x: 0, y: 20, width: screenSize.width, height: mDimen.mScrollBarH)
        ...
        //创建底部线条  
        let lineRect : CGRect = CGRect.init(x: 0,y: 40, width: isScroll == true ? self.btnList[0].frame.size.width : averageItemWidth,
                                            height: 1.5)  
