...
class PDFScrollView: UIScrollView,UIScrollViewDelegate{
    ...
    func initialize(){
        if let page = PDF.page(at: 1){
            ...
            delegate = self
            addSubview(PDFview)
            self.setContentOffset(CGPoint(x:CGFloat(0),y:CGFloat(pdfRect.height*CGFloat(mBookStratPage-1)+CGFloat(mBookStratPage-1)*10*pdfScale)), animated: true)//设置起始PDF位置
        }
    }
