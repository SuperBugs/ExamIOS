...
import UIKit
class UserCollectSubjectsViewController:UIViewController{
    ...
    var mController:CollectSubjectViewController!//上一个viewcontroller
    ...
    ///获取上一个viewcontroller
    public func setController(controller:CollectSubjectViewController){
    ...
    @objc func bottomCollectTiMu() {
        ...
        let contioner:UIScrollView=scrollView.subviews[page] as! UIScrollView
        let tiMuId=mTiMuIds[contioner.tag]//题目ID
        var collectTiMus=[Int]()
        
        
        if let collectTiMu = CommonFunction.getNormalDefult(key: "公共基础collect") as? [Int]{
            collectTiMus=collectTiMu
            if(collectTiMus.contains(tiMuId)){
                collectTiMus.remove(at: collectTiMus.index(of: tiMuId)!)
                CommonFunction.setNormalDefault(key: "公共基础collect", value: collectTiMus as AnyObject)
                
            }
        }
        if let collectTiMu = CommonFunction.getNormalDefult(key: "道路工程collect") as? [Int]{
            collectTiMus=collectTiMu
            if(collectTiMus.contains(tiMuId)){
                collectTiMus.remove(at: collectTiMus.index(of: tiMuId)!)
                CommonFunction.setNormalDefault(key: "道路工程collect", value: collectTiMus as AnyObject)
                
            }
        }
        if let collectTiMu = CommonFunction.getNormalDefult(key: "桥梁隧道工程collect") as? [Int]{
            collectTiMus=collectTiMu
            if(collectTiMus.contains(tiMuId)){
                collectTiMus.remove(at: collectTiMus.index(of: tiMuId)!)
                CommonFunction.setNormalDefault(key: "桥梁隧道工程collect", value: collectTiMus as AnyObject)
                
            }
        }
        if let collectTiMu = CommonFunction.getNormalDefult(key: "交通工程collect") as? [Int]{
            collectTiMus=collectTiMu
            if(collectTiMus.contains(tiMuId)){
                collectTiMus.remove(at: collectTiMus.index(of: tiMuId)!)
                CommonFunction.setNormalDefault(key: "交通工程collect", value: collectTiMus as AnyObject)
                
            }
        }
        ToastView.showInfo(info: "取消收藏成功", bgColor: UIColor.white,inView:self.view,vertical: 0.8)
...
extension UserCollectSubjectsViewController: UIScrollViewDelegate {
