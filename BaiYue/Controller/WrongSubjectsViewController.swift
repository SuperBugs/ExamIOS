//
//  WrongSubjectsViewController.swift
//  BaiYue
//
...
import UIKit
class WrongSubjectsViewController:UIViewController{
    ...
    var mAllTiMuIds = [Int]()
    var mAllTiMuData=[[String:Any]]()//所有题目数据
    var mTiMuItems=[Int]()//错题序号
    var mTiMuModul=TiMuModule()//题目的模型
    var mTiMuData=[[String:Any]]()//错题数据
    var mTiMuNum=0//当前题号
    var mTiMuXuHao=0//当前题号
    ...
    public func setTiMuIdsData(tiMuIds:[Int]){
        mTiMuIds=tiMuIds
        //print("mTiMuIds+\(mTiMuIds)")
    }
    ///题目序号
    public func setTiMuItemData(tiMuItems:[Int]){
        mTiMuItems=tiMuItems
        //print("mTiMuItems+\(mTiMuItems)")
    }
    ///从上一个类获取题目数据,是完整数据
    public func setTiMusData(tiMusData:[[String : Any]]){
        mAllTiMuData=tiMusData
    }
    public func setAllTiMuIdsData(allTiMuIds: [Int]){
        mAllTiMuIds=allTiMuIds
    }
    ...
    func setupViews() {
        ...
        do {
            for index in 0..<mTiMuIds.count {
                let data=mAllTiMuData[mTiMuItems[index]]
                mTiMuData.insert(data, at: index)
            }
            //print("mTiMuData+\(mTiMuData)")
            for index in 0..<mTiMuIds.count {
                if((mTiMuData[index]["class_timu"] as! String)=="单选题"){
                    setDanXuanTiView(tiMuDataId:index)//设置单选题的视图
                    mTiMuXuHao=mTiMuIdAndItems[mAllTiMuIds.index(of: mTiMuIds[index])!]
    ...
    public func setDanXuanTiView(tiMuDataId:Int){
        ...
        let tiMu="\(mTiMuXuHao)、\(String(describing: mTiMuData[tiMuDataId]["timu"]!))"
        ...
        if((mTiMuData[tiMuDataId]["sc_state"] as! Int)==0){
            ...
            if(String(describing: mTiMuData[container.tag]["class_timu"]!)=="单选题"){
                ...
                if let userDaan = data["userdaan_ks"] as? String{
    ...
    @objc func clickDuoXuanTiA(){
        ...
        let contioner=scrollView.subviews[page]
        let a=contioner.subviews[4] as! UIImageView
        if((mTiMuData[contioner.tag]["sc_state"] as! Int)==1){
    ...
    @objc func bottomCheckAnswer() {
        ...
        if(String(describing: mTiMuData[contioner.tag]["class_timu"]!)=="单选题"){
            ...
            if let userDaan = data[0]["userdaan_ks"] as? String{
    ...
    @objc func toDaTiKa(){
        ...
        bottomCheckAnswer()
        let daTiKaC=WrongDaTiKaViewController()
        daTiKaC.setTiMuIdsData(tiMuIds: mTiMuIds)
        daTiKaC.setTiMuData(tiMuData:mTiMuData)
        daTiKaC.setTiMuIdAndItems(idAndItems: mTiMuIdAndItems)
        daTiKaC.setAllTiMuIdsData(allTiMuIds: mAllTiMuIds)
...
extension WrongSubjectsViewController: UIScrollViewDelegate {
